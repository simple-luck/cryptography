...
GFMul(int n, int s)
{
  ...
  while (n!=0) {
    if (n & 1u) {//若n的最后一位为1
      result = (result ^ sum);//异或，GF（2）中，加法等价于异或运算
    }
    n = (n>>1);
    sum = GFMul2(sum);
  }
	...
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			array[i][j]=GFMul(colM[i][0],temp[0][j])^
			            GFMul(colM[i][1],temp[1][j])^
			            GFMul(colM[i][2],temp[2][j])^
			            GFMul(colM[i][3],temp[3][j]);
		}
	}
	...
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			array[i][j]=GFMul(deColM[i][0],temp[0][j])^
			            GFMul(deColM[i][1],temp[1][j])^
			            GFMul(deColM[i][2],temp[2][j])^
			            GFMul(deColM[i][3],temp[3][j]);
		}
	}
}
...
void addRoundKey(int array[4][4], int round)
{
  //请补充代码
    int key[4];
	for(int i=0;i<4;i++){
		splitIntToArray(w[round*4+i],key);
		for(int j=0;j<4;j++){
			array[j][i]^=key[j];
		}
	}
...
//请补充代码
    int a[4];
	splitIntToArray(num,a);
	leftLoop4int(a,1);

	for(int i=0;i<4;i++){
		a[i]=getNumFromSBox(a[i]);
	}
	int result=mergeArrayToInt(a);
...
void deAes(char *c, int clen, char *key)
{
    ...
    for(k = 0; k < clen; k += 16)
    {
        ...
        addRoundKey(cArray, 10); // 初始轮密钥加（加上第10轮密钥）
        ...
        addRoundKey(cArray, 0);  // 最后一轮的轮密钥加

        convertArrayToStr(cArray, c + k);  // 将数组转换回字符并保存到输出中
    }
}
/**
*读取字符串
